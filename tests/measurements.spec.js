...
test('stock search CSV and backup preserve inches and reject malformed imported measurements',async({page})=>{
 await boot(page,measured());await nav(page,'stock');await page.locator('#stock-search').fill('อก 22.5');await expect(page.locator('[data-stock-variant]:visible')).toHaveCount(1);const csv=await download(page,'#stock-export-filtered');expect(csv).toContain('อก (นิ้ว)');expect(csv).toContain('22.5');expect(csv).not.toContain('variant-2');await page.locator('.data-tools summary').click();const backup=JSON.parse(await download(page,'#export-btn'));expect(backup.version).toBe(10);const upload=async data=>page.locator('#import-input').setInputFiles({name:'measurements.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(data))});await upload(backup);await page.locator('#modal-ok-btn').click();await expectSaved(page,s=>s.products[0].variants[0].chestInches===22.5);
