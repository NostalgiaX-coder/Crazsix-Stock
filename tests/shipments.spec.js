...
test('shipment backups round trip; inconsistent totals, missing sales and forged status are rejected',async({page})=>{
 ...
 await page.locator('.data-tools summary').click();const backup=JSON.parse(await download(page,'#export-btn'));expect(backup.version).toBe(10);const upload=data=>page.locator('#import-input').setInputFiles({name:'shipping.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(data))});
